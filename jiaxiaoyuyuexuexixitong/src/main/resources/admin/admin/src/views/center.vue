  ...
  <div>
    ...
    >  
     <el-row>
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='users'" label='用户头像' prop="usersPhoto">
                 <file-upload
                         tip="点击上传照片"
                         action="file/upload"
                         :limit="3"
                         :multiple="true"
                         :fileUrls="ruleForm.usersPhoto?$base.url+ruleForm.usersPhoto:''"
...
|| this.flag=='jiaolian')){
      ...
      if('users' == this.flag && this.ruleForm.usersPhoto) {
        this.$http({
          url: `${this.$base.url}users/updatePhoto`,
          method: "post",
          data: { usersPhoto: this.ruleForm.usersPhoto }
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.$http({
              url: `${this.$storage.get("sessionTable")}/update`,
