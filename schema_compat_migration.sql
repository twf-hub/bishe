...
RENAME TABLE `gonggao` TO `notice`;
...
RENAME TABLE `examrecord` TO `exam_record`;
RENAME TABLE `examredetails` TO `exam_detail`;
RENAME TABLE `examrewrongquestion` TO `wrong_question`;
RENAME TABLE `exampaper` TO `exam_paper`;
RENAME TABLE `exampapertopic` TO `exam_paper_question`;
RENAME TABLE `examquestion` TO `exam_question`;
...
CREATE OR REPLACE VIEW `gonggao` AS
SELECT
  `id`,
  `title` AS `gonggao_name`,
  `image` AS `gonggao_photo`,
  `types` AS `gonggao_types`,
  `insert_time`,
  `content` AS `gonggao_content`,
  `create_time`
FROM `notice`;
...
SELECT
  ...
  `username`,
  `password`,
  `name` AS `jiaolian_name`,
  `phone` AS `jiaolian_phone`,
  `id_card` AS `jiaolian_id_number`,
  `photo` AS `jiaolian_photo`,
  `sex_type` AS `sex_types`,
  `email` AS `jiaolian_email`,
  `create_time`
FROM `coach`;
...
SELECT
  `id`,
  `registration_number` AS `jiaolian_yuyue_uuid_number`,
  `coach_id` AS `jiaolian_id`,
  `user_id` AS `yonghu_id`,
  `reason_text` AS `jiaolian_yuyue_text`,
  `insert_time`,
  `status` AS `jiaolian_yuyue_yesno_types`,
  `reply_text` AS `jiaolian_yuyue_yesno_text`,
  `review_time` AS `jiaolian_yuyue_shenhe_time`,
  `appointment_time` AS `jiaolian_yuyue_time`,
  `create_time`
...
SELECT
  ...
  `username`,
  `password`,
  `user_name` AS `yonghu_name`,
  `user_phone` AS `yonghu_phone`,
  `user_id_number` AS `yonghu_id_number`,
  `user_photo` AS `yonghu_photo`,
  `sex_type` AS `sex_types`,
  `user_email` AS `yonghu_email`,
  `create_time`
FROM `user`;
...
SELECT
  `id`,
  `material_name` AS `ziliao_name`,
  `material_number` AS `ziliao_uuid_number`,
  `material_photo` AS `ziliao_photo`,
  `good_number` AS `zan_number`,
  `bad_number` AS `cai_number`,
  `material_types` AS `ziliao_types`,
  `material_video` AS `ziliao_video`,
  `file_url` AS `ziliao_file`,
  `material_content` AS `ziliao_content`,
  `material_delete` AS `ziliao_delete`,
  `insert_time`,
  `create_time`
FROM `study_material`;
...
SELECT
  `id`,
  `material_id` AS `ziliao_id`,
  `user_id` AS `yonghu_id`,
  `material_types` AS `ziliao_collection_types`,
  `insert_time`,
  `create_time`
...
SELECT
  ...
  `material_id` AS `ziliao_id`,
  `user_id` AS `yonghu_id`,
  `content` AS `ziliao_liuyan_text`,
  `insert_time`,
  `reply_text`,
  `reply_time` AS `update_time`,
  `create_time`
...
SELECT
  `id`,
  `user_id` AS `yonghu_id`,
  `title` AS `liuyan_name`,
  `content` AS `liuyan_text`,
  `insert_time`,
  `reply_text`,
  `reply_time` AS `update_time`,
  `create_time`
...
SELECT
  ...
  `user_id` AS `yonghu_id`,
  `question_id` AS `examquestion_id`,
  `user_answer` AS `examredetails_myanswer`,
  `score` AS `examredetails_myscore`,
  `create_time`
FROM `exam_detail`;
...
SELECT
  `id`,
  `exam_paper_name` AS `exampaper_name`,
  `duration` AS `exampaper_date`,
  `total_score` AS `exampaper_myscore`,
  `subject_type` AS `kemu_types`,
  `status` AS `exampaper_types`,
  `method` AS `zujuan_types`,
  `is_delete` AS `exampaper_delete`,
  `create_time`
FROM `exam_paper`;
...
SELECT
  `id`,
  `exam_paper_id` AS `exampaper_id`,
  `question_id` AS `examquestion_id`,
  `question_score` AS `exampapertopic_number`,
  `create_time`
...
SELECT
  `id`,
  `paper_name` AS `examquestion_name`,
  `subject_type` AS `kemu_types`,
  `question_options` AS `examquestion_options`,
  `question_answer` AS `examquestion_answer`,
  `question_analysis` AS `examquestion_analysis`,
  `question_type` AS `examquestion_types`,
  `sort` AS `examquestion_sequence`,
  `create_time`
FROM `exam_question`;
